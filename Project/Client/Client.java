...
public enum Client {
    ...
    private boolean processClientCommand(String text) {
        ...
        // UCID - ob75 - March 27, 2024
            ...
            String rollGame = text.replace(ROLL, "").trim();
            if (rollGame.contains("d")) {
                String[] vals = rollGame.split("d");
                if (vals.length >= 2) {
                    try {
                        int dice = Integer.parseInt(vals[0]);
                ...
                try {
                    int dice = 0;
                    int sides = Integer.parseInt(rollGame);
                    sendRoll(dice, sides);
    ...
    // UCID - ob75 - March 30, 2024
    private void sendRoll(int dice, int sides) throws IOException {
